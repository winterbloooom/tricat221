...
class Autonomous:
    ...
    def calc_psi_goal(self):
        self.psi_goal = math.degrees(math.atan2(self.goal_y - self.boat_y, self.goal_x - self.boat_x))
    ...
    def show(self, error_angle, u_servo, visualize=False):
        ...
        if visualize:
            # 목표 지점
            goal = visual.point_rviz(name="goal", id=2, x=self.goal_x, y=self.goal_y, color_b=255, scale=0.2)
            # 지나온 경로
            ...
            psi = visual.arrow_rviz(name="psi", id=4, x1=self.boat_x, y1=self.boat_y, x2=psi_arrow_end_x, y2=psi_arrow_end_y, color_r=221, color_g=119, color_b=252)
            psi_txt = visual.text_rviz(name="psi", id=8, text="psi", x=psi_arrow_end_x, y=psi_arrow_end_y)
            # psi_desire
            ...
            psi_desire = visual.arrow_rviz(name="psi_desire", id=5, x1=self.boat_x, y1=self.boat_y, x2=desire_arrow_end_x, y2=desire_arrow_end_y, color_r = 59, color_g=139, color_b=245)
            psi_desire_txt = visual.text_rviz(name="psi_desire", id=9, text="desire", x=desire_arrow_end_x, y=desire_arrow_end_y)
            # 골까지
            goal_line = visual.linelist_rviz(name="goal_line", id=6, lines=[[self.boat_x, self.boat_y], [self.goal_x, self.goal_y]], color_b=255, scale=0.04)
            # angle_range
            ## 지금은 스킵
            # inrange obs
            ...
            obstacles = visual.linelist_rviz(name="obs", id=7, lines=inrange_obs_world, color_r=237, color_g=234, color_b=74)
            # axis
            axis_x = visual.linelist_rviz(name="axis_x", id=11, lines=[[self.boat_x, self.boat_y], [self.boat_x + 3, self.boat_y]], color_r=255, scale=0.1)
            axis_y = visual.linelist_rviz(name="axis_x", id=12, lines=[[self.boat_x, self.boat_y], [self.boat_x, self.boat_y+3]], color_g=255, scale=0.1)
            all_markers = visual.marker_array_rviz([goal, psi, psi_txt, traj, psi_desire, psi_desire_txt, goal_line, obstacles, axis_x, axis_y])
    ...
    def degree_to_servo(self, error_angle):
        ...
        u_servo = (u_angle - self.ob_angle_range[0]) * (self.servo_range[1] - self.servo_range[0]) / (
            self.ob_angle_range[1] - self.ob_angle_range[0]
        ) + self.servo_range[0]
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        if arrived: # 최종 목적지 도달함
            ...
            auto.inrange_obstacles, auto.danger_angles = oa.ob_filtering(auto.obstacles, auto.span_angle, auto.ob_angle_range, auto.ob_dist_range)
            auto.calc_psi_goal()
            error_angle = oa.calc_desire_angle(auto.danger_angles, auto.psi_goal - auto.psi, auto.ob_angle_range) # 목표각과 현 헤딩 사이 상대적 각도
